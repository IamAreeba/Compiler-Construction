...
# ===============  Mine Work  ==================
            ...
            if self.get_next_token() and self.current_token[0] == 'ID':
                ...
                if self.get_next_token() and self.current_token[0] == '(':
                    ...
                    if self.get_next_token() and self.current_token[0] == ')':
                        ...
                        # TE = PL + “->”+ T
                        # # ID must equal to the class name
                        # if(!insertFunMT(N, TE, AM , TM, RefMT)):
                        #     “Redeclaration Error”
    ...
    def Z1(self):
        if self.current_token[0] == '{':
            if self.get_next_token() and self.Mst():
                ...
                # check if this function is declared with same
                # signature in the parent class with ‘=0’ then it is valid
    ...
    def X(self):
        ...
        # If(CR=NULL):
        #   T=lookUpST(N)
        #   //maybe needs to check in parents scopes
        #   If(T==NULL):
        #      RMT=lookUpMT(N,CCR)
        #      If(RMT==NULL):
        #          “Undeclared”
        #   T=RMT.Type
        #   If(RMT.AM==”secure”)
        #       “This Variable cannot be accessed outside of the class”
        #   Else if(CR=CCR)
        #       RMT=lookUpMT(N,CR)
        #       If(RMT==NULL)
        #           “Undeclared”
        ...
        if self.current_token[0] == '.':
            ...
            if self.get_next_token() and self.Al():
                if self.current_token[0] == ')':
                    ...
                    # If(CR==NULL)
                    #   RMT=LookUpFunMT(N,PL,CCR)
                    #   If(RMT==NULL)
                    #   //it should check in parent classes as well
                    #   T=lookUpFunST(N,PL)
                    #   If(T==NULL)
                    #       “Undeclared”
                    #   T=RMT.Type
                    # If(CR==CCR)
                    #   RMT=lookUpFunMT(N,PL,CR)
                    # If(RMT==NULL)
                    #   “Undeclared”
                    # T=RMT.Type
